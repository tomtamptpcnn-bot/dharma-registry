...
วันที่แสดงวัน/เดือน/ปี พ.ศ. ทั้งปฏิทิน ตัวกรอง รายละเอียด และใบพิมพ์ โดยเก็บเป็น PostgreSQL date ปี ค.ศ. เพื่อคำนวณและค้นหาวันที่ได้ถูกต้อง ช่องอาจารย์แนะนำ อาจารย์รับรอง อาจารย์ถ่ายทอดเบิกธรรม ชั้น ระดับ และสถานที่รับธรรมเลือกจากข้อมูลตัวเลือกที่ผู้ดูแลสร้าง จำนวนเงินรองรับทศนิยม 2 ตำแหน่ง
...
เข้าเมนู **จัดการข้อมูลตัวเลือก** เพื่อเพิ่มรายการในทั้ง 6 หมวด (หลังรัน migration ล่าสุด) หรือกด **+** ข้างช่องเลือกในฟอร์มเพื่อเพิ่มและเลือกทันที รองรับค้นหาและล้างค่า ตัวเลือกที่เพิ่มจะใช้ร่วมกันทั้งระบบ สามารถแก้ไขชื่อโดยอัปเดตทะเบียนที่เกี่ยวข้องพร้อมกัน และลบได้เฉพาะตัวเลือกที่ยังไม่มีทะเบียนใช้งาน
...
รัน `supabase/migrations/202609230001_transmitting_teacher.sql` ใน Supabase SQL Editor หลัง migration เดิม ก่อนใช้โปรแกรมรุ่นนี้ เพื่อเพิ่มอาจารย์ถ่ายทอดเบิกธรรมและรองรับการเพิ่ม/เปลี่ยนชื่อ/ลบตัวเลือกในหมวดใหม่

ที่อยู่เลือกจังหวัด → อำเภอ/เขต → ตำบล/แขวง และเติมรหัสไปรษณีย์อัตโนมัติ บ้านเลขที่/หมู่บ้าน/ถนนกรอกเอง ที่อยู่เก่าที่ระบุจังหวัด/อำเภอ/ตำบลชัดเจนจะแยกเป็นตัวเลือกเมื่อเปิดแก้ไข รองรับชื่อเต็ม ชื่อย่อ และข้อความติดกัน ส่วนที่มีเพียงชื่อหมู่บ้านหรือข้อมูลไม่ตรงกับชุดตัวเลือกจะเก็บไว้ในรายละเอียดโดยไม่เดาจังหวัดให้ ข้อมูลตัวเลือกแนบอยู่ในโปรแกรม ไม่ต้องเรียกบริการภายนอกเมื่อกรอกที่อยู่ แหล่งข้อมูลและใบอนุญาตอยู่ใน `lib/data/`.

## ชื่อข้อมูลตัวเลือกไม่ซ้ำ (ทั้ง 6 หมวด)

หลัง migration `202609230001_transmitting_teacher.sql` ให้รัน `supabase/migrations/202609230002_case_insensitive_registry_options.sql` ใน Supabase SQL Editor ด้วย

- เพิ่มและแก้ไขชื่อในหมวดเดียวกันห้ามซ้ำ โดยไม่แยกตัวพิมพ์เล็ก–ใหญ่และช่องว่างหัวท้าย เช่น `Level A`, `level a` และ ` level a ` ถือเป็นชื่อเดียวกัน ชื่อเหมือนกันคนละหมวดยังใช้ได้
- หากมีตัวเลือกซ้ำอยู่เดิม migration จะเก็บรายการที่สร้างก่อน และย้ายทะเบียนทั้งหมดไปอ้างอิงชื่อนั้นก่อนลบตัวเลือกซ้ำ โดยไม่ลบทะเบียน
- แก้ไขชื่อได้แม้มีทะเบียนใช้งาน ระบบเปลี่ยนชื่อในทะเบียนที่เกี่ยวข้องพร้อมกัน
- ปุ่มลบปิดเมื่อมีทะเบียนใช้งาน และฐานข้อมูลตรวจซ้ำพร้อมล็อกการเขียนทะเบียนระหว่างการแก้ไข/ลบ ป้องกันข้อมูลอ้างอิงหายเมื่อมีผู้ใช้หลายคน

### เวลาที่รับธรรม

รัน `supabase/migrations/202609250001_recipient_received_time.sql` ใน Supabase SQL Editor หลัง migration เดิม ก่อนใช้ฟอร์มรุ่นนี้ เพื่อเพิ่มช่องเวลาที่รับธรรม (HH:mm แบบ 24 ชั่วโมง) ไม่บังคับกรอก ข้อมูลเดิมจะมีค่าเป็น NULL และเวลาจะไม่ถูกแปลงเขตเวลา

รัน `supabase/migrations/202609250002_recipient_received_time_range.sql` ต่อเพื่อรองรับช่วงเวลา เช่น 13:00–15:00 หากกรอกต้องระบุทั้งเวลาเริ่มและเวลาสิ้นสุด โดยเวลาสิ้นสุดต้องหลังเวลาเริ่มในวันเดียวกัน เวลาที่บันทึกไว้เดิมจะคงเป็นเวลาเริ่ม

ฟอร์มเลือกช่วงเวลาผ่าน Select ได้ โดยมีตัวเลือกเริ่มต้น 13:00–15:00 และ “กำหนดเอง” สำหรับกรอกเวลาเริ่ม–สิ้นสุดอื่น ๆ สามารถล้างค่าเพื่อไม่ระบุช่วงเวลาได้

รัน `supabase/migrations/202609250003_set_all_recipient_time_ranges.sql` หลังสองไฟล์ข้างต้น เพื่อตั้งช่วงเวลาเป็น 13:00–15:00 ให้ผู้รับธรรมะทุกคนที่มีอยู่ขณะรัน รวมทับเวลาเดิม รายการที่เป็นช่วงเวลานี้อยู่แล้วจะไม่ถูกอัปเดตซ้ำ
